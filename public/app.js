...
app.factory('imageBankGlobalSpace',function(){
  ...
  var conf = {
    columnPrefix : 'ibColumn-',
    columnCssClass : 'imageBankColumn',
    containerID : 'iBankContainer'
  };
  ...
  var _this = {
    ...
    preTraitement : function(){
      ...
      if (screenWidth < 670) {
        ...
        ColumnsNbr = Math.floor(screenWidth / 223);
      }
      columnWidth = 100 / ColumnsNbr;

      for (var i = 0; i < ColumnsNbr; i++) {
        columnsHeight.push(0);
        var element = $('<div id="'+(conf.columnPrefix+i)+'" class="'+conf.columnCssClass+'" style="width : '+columnWidth+'%;"></div>');
        $('#'+conf.containerID).append(element);
      }
    },
    getIndexColumn : function(){
        var index = 0;
        for (var i = 1; i < ColumnsNbr; i++) {
          if(columnsHeight[i]<columnsHeight[index])
              index = i;
        }
        return index;
    },
    traiteImage : function(img){
      // var handleImage = function(height,url,i,n){
        // if(i<n){
          img.remove();
          var index =  _this.getIndexColumn();
          var element = $('<div class="imageBankItem"></div>');
          element.append(img);
          $('#'+conf.columnPrefix+index).append(element);
          columnsHeight[index] += element.height();
    },
  ...
  return _this;
}).directive('imageBankDir',['imageBankGlobalSpace',function(imageBankGlobalSpace){
  return {
    link : function(scope,element,attrs){
      if(scope.$first){
        imageBankGlobalSpace.preTraitement();
      }
      if(scope.$last){

      }
      imageBankGlobalSpace.traiteImage(element);
    }
  }
}]).controller('imageBankCtrl',function($scope,$timeout,imageBankGlobalSpace){
